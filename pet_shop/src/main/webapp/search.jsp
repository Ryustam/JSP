...
<html lang="en">
    ...
    <%!
    	...
    	private ArrayList<goodsDTO> search_dtos;
    %>
	...
	int count=0;
	count=goods.getSearchCount(search);
                ...
                <div class="col-lg-8">
                    ...
                    <div class="row">
                        <div class="col-lg-6">
                            <!-- Blog post-->
                            <%
                            	search_dtos=goods.getSearch(currentPage,search);
                            	for(int i=0;i<search_dtos.size();i++) {
                            		if(i==2){
                            			break;
                            		}
                            		
                            		System.out.println(search_dtos.get(i).getName());
                            		
                            %>
                            <div class="card mb-4">
                                <a href="#!"><img class="card-img-top" src="./resources/image/<%=search_dtos.get(i).getName()+".jpg"%>" width="700px" height="350"alt="..." /></a>
                                <div class="card-body">
                                    <!-- <div class="small text-muted">January 1, 2021</div>-->
                                    <h2 class="card-title h4"><%=search_dtos.get(i).getName()%></h2>
                                    <p class="card-text"><%=search_dtos.get(i).getPrice()%>원</p>
                                    <%
                                    	if(login==true){
                                    %>
                                    <form action="./select_search.jsp" method="post">
                                    <input type="hidden" name="goodName" value="<%=search_dtos.get(i).getName()%>">
                                    <input type="hidden" name="goodPrice" value="<%=search_dtos.get(i).getPrice()%>">
                        ...
                        <div class="col-lg-6">
                            <!-- Blog post-->
                            <%
                            	for(int i=2;i<search_dtos.size();i++) {
                    ...
                    <nav aria-label="Pagination">
                        ...
                        <ul class="pagination justify-content-center my-4">
                            <%
                        		if(currentPagi>0){
                        	%>
                        		<li class="page-item"><a class="page-link" href="./search.jsp?pagi_Num=<%=currentPagi-1%>&search=<%=URLEncoder.encode(search,"UTF-8")%>" tabindex="-1" aria-disabled="true"><</a></li>
                        	<%
                        		} else {
                        	%>
                            ...
                            %>
                            <li class="page-item active" aria-current="page"><a class="page-link" href="./search.jsp?pagi_Num=<%=currentPagi%>&pageNum=<%=i%>&search=<%=URLEncoder.encode(search,"UTF-8")%>"><%=i%></a></li>
                           	<%
                           			if(i%5==0){
                    					break;
                    				}
                            ...
                            %>
                            <li class="page-item"><a class="page-link" href="./search.jsp?pagi_Num=<%=currentPagi+1%>&pageNum=<%=1+((currentPagi+1)*5)%>&search=<%=URLEncoder.encode(search,"UTF-8")%>">></a></li>
                        	<% 
                            	}
                        	%>
                        </ul>
